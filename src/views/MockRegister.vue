...
<form method="post" @submit.prevent="handleSubmit($event)" style="box-shadow: 10px 10px 1px rgba(0, 0, 0, 0.5)"   class="w-1/3 h-auto flex flex-col items-center  bg-white rounded-b-md drop-shadow-lg">
     ...
     <h1 class="text-white font-bold text-4xl mb-5 bg-gradient-to-r bg-[rgb(38,65,86)] p-4 w-full rounded-t-md">Register</h1>
        ...
        <div class="w-3/4 mb-3 rounded-sm shadow flex flex-col space-y-2 p-3" >
            <input placeholder="email" v-model="mailAddress" id="mailAddress" name="mailAddress" class="p-1 border border-solid border-black"/>

            <input placeholder="userType" v-model="userType" id="userType" name="userType" class="p-1 border border-solid border-black"/>
